...
namespace ClearOffice.Hrm.Business
{
    public class TrainingProxy
    {
        ...
        {
            foreach (
                var entry in context.ObjectStateManager.GetObjectStateEntries(EntityState.Added | EntityState.Modified))
            {
        ...
        {
            return
                context.ObjectStateManager.GetObjectStateEntries(EntityState.Added | EntityState.Modified).Any(
                    entry => !entry.IsRelationship && (entry.Entity.GetType() == entity.GetType()));
        }
