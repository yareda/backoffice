...
FROM         fleet.maintanance INNER JOIN
                      fleet.maintenance_activity ON fleet.maintanance.maintenance_id = fleet.maintenance_activity.maintenance_id INNER JOIN
                      fleet.maintenance_part ON fleet.maintanance.maintenance_id = fleet.maintenance_part.maintenance_id INNER JOIN
                      fleet.vehicle_info ON fleet.maintanance.vehicle_id = fleet.vehicle_info.vehicle_id INNER JOIN
                      fleet.maintenance_expense_info ON fleet.maintanance.maintenance_id = fleet.maintenance_expense_info.maintenance_id
